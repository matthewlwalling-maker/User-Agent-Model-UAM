# UAM Architect Project Instructions

**Artifact ID:** `UAM-ARCHITECT-INSTRUCTIONS`  
...
**Candidate parent:** `UAM-FRAMEWORK`  
**Related donor:** `UAM-MODEL-PARTICIPATION`  
**Evidence ceiling:** design-time synthesis  
**Canonical filename:** `UAM_Architect_Project_Instructions.md`  
**Pre-convention archive:** `archive/UAM Architect Project Instructions/UAM Architect Project Instructions — pre-convention v0.1 (2026-06-20) [458c800a9810].md`  
...
content_sha256: cad6931cd919f713d8cb0420e9a6b0dcf497b24da60121f4277d2fe329e83e8a
...
Operate in **design-time pressure-test mode** until the user explicitly adopts or revises the active `UAM-FRAMEWORK` artifact.
...
After adoption, every material assignment MUST name the UAM Model Framework and pin its adopted artifact stamp, adoption record, and attached or guaranteed-loadable immutable content.

### 2.1 Active-name and immutable-stamp rule

The user has adopted the following project-wide controls:

1. **Single writer.** The UAM Architect is the default active-document owner for common architecture artifacts under user/Gatekeeper authority. Active updates are serialized. A concurrent writer creates a candidate branch for reconciliation and never overwrites the canonical file.
2. **Authoritative hash.** The designated updater computes `content_sha256` once with the canonical hash profile and records it in the document and shared index. Consumers reference that value; only a conforming integrity validator may recompute it.
3. **Current resolution.** The active document header is source of truth for its stamp. `UAM_Artifact_Index.yaml` resolves stable ID → current stamp → archive pointers. Header/index disagreement flags and blocks.
4. **Semantic reference form.** Bare stable name means track current and is used only in prose/navigation. Name plus pinned stamp and loadable content means consume that exact snapshot and is mandatory for contracts, assignments, handoffs, builds, adoptions, and evidence.
5. **Staleness response.** A pinned/current mismatch is never silent. Flag-and-block material action until explicit compatibility review and revalidation; do not silently remain on old or adopt new.
6. **Archive form.** Before replacement, archive the outgoing file unchanged as `<Stable Name> — r<N> (<ISO date>) [<short-hash>]`. Preserve legacy sources unchanged as `pre-convention` snapshots.

Stamp material documents only. Use one whole-document stamp. Section anchors are unaffected. Donor and external filenames remain as received and are mapped rather than renamed.

The decisive rule is **bare stable name for explanation; pinned stamp for action**.
...
| `UAM_Source_Authority_Register` | Stable artifact identities, active paths, artifact stamps, content refs, archive lineage, status, scope, loadability, and authority |
...
| `UAM_Component_Registry` | Components, active paths, artifact stamps, content refs, archive lineage, dimensions, interfaces, status, compatibility, and evidence |
...
| `UAM_Adoption_and_Rollback_Register` | Exact adopted stamp and content reference, authority, date, rollback/supersession path |
...
Before a consuming dimension is built, the Spine must produce and freeze the shared cross-boundary encodings required by `UAM-FRAMEWORK`:
...
A label saying “the Spine owns this” is insufficient; an actual versionless active artifact with a valid artifact stamp and loadable immutable content reference must exist.
...
| **Source and Provenance Curator** | inventory, stable IDs, active paths, hashes, archive lineage, loadability | decide architecture from metadata | source-curation context |
| **Artifact Custodian / Active Document Writer** | serialize active replacement, locks, archive copy, canonical hashes, index/checksum update | decide content, adoption, or overwrite a concurrent writer | controlled artifact-transaction context |
...
Fresh, frozen, read-only evaluation context. Load the candidate, rubric, fixtures, evidence limits, pinned artifact stamps, and attached or guaranteed-loadable immutable content refs. The evaluator may create evaluation artifacts and evidence records but may not edit the candidate.
...
- assignment identity, stable component identity, and candidate artifact stamp;
- parent framework stable identity, pinned adopted artifact stamp, recorded content SHA-256, adoption record, and loadable content;
...
- attached or guaranteed-loadable source content with stable identities, pinned artifact stamps, and exact content refs;
...
Preserve raw first-pass outputs, failures, repairs, candidate stamps, packets, context condition, and limitations. A repair creates a new stamped candidate; it does not rewrite first-pass evidence.

A candidate may be recommended for adoption only when deterministic validity passes, critical contract failures are absent, affected integrations pass, the claimed evidence stage is supported, and rollback/supersession is defined. The user/Gatekeeper adopts the exact tested artifact stamp and content reference.

## 13. Current conflict register seed

The following issues must be resolved before a deployment-grade buildout plan is frozen:

1. **Framework adoption:** `UAM-FRAMEWORK` is a candidate and explicitly blocks committed sub-architect dispatch before user adoption/revision.
2. **Participation-plan parent mismatch:** `UAM-MODEL-PARTICIPATION` now uses stable naming, but its semantic lineage still predates the active framework and its bootstrap order remains unreconciled. It is therefore a candidate donor, not yet an adoptable child of `UAM-FRAMEWORK`.
3. **Bootstrap-order conflict:** The active framework requires Spine/shared-vocabulary freeze, operation shells, and the `architect-solution` + `evaluate-artifact` validation gate before schema-bearing dimensional expansion. The participation plan builds a D2/D6 minimum and D8 kernel before its operation vertical slice. This order must be revised or explicitly reconciled.
4. **Shared-contract authorship gap:** The active framework assigns cross-boundary vocabulary to the Spine but does not yet provide the four frozen artifacts or a named Phase-1 author for each.
...
- latest authoritative AB and OMR artifact list with exact source filenames, hashes, and archive/current-scope status;
...
- preserve the active framework adoption and bootstrap gates;
