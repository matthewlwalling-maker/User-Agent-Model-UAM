...
- current artifact references and exact snapshot hashes/archive refs;
...
- identity;
- schema or contract identity and snapshot hash;
- revision;
- branch;
- parent references;
- content hash;
...
4. **Packet / manifest format** — the transport-envelope contract (produced by D3/D7, consumed by every operation that hands off).
...
5. **Artifact linkage** — exact artifact identity and snapshot hash where material.
...
5. **Manifest and integrity** — inventory, stable IDs, snapshot hashes, completeness, and dependencies.
...
4. What inputs, snapshot hashes or archive refs, state, and evidence does it require?
