...
**Status:** active project instructions for design-time pressure testing; not runtime authority  
...
**Source placement:** project source material root  
...
3. **Current resolution.** The active document header is source of truth for its stamp. `UAM_Artifact_Index.yaml` resolves stable ID → canonical active path → current stamp → archive pointers. Header/index disagreement flags and blocks.
