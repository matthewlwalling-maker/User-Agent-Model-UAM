...
artifact_stamp:
  ...
  content_sha256: bd07fd9144ef835e7ac797e28015537dcaced5d1beb812836565975358925f55
...
- Optimize for the live project objective, not checklist performance.
...
- Treat invariants as binding, but treat procedures, ledgers, checklists, and output templates as decision aids unless the governing source marks them as hard gates.
- Use the smallest process that can protect the project objective, authority, evidence, and work-object boundaries.
- Preserve objective agent reasoning: when a rule appears to distort the current goal, surface the tension, follow the higher-authority source, and choose the narrowest reversible move.
- Do not convert the bridge into ritual. A simple answer, a no-material-change result, or a stop-for-alignment decision is correct when it best serves the objective.
...
2. This `AGENTS.md`.
3. `FOR CONTEXT/UAM_Bridge_Skills_New_Project_Handoff.md`.
4. `FOR CONTEXT/UAM_Bridge_Skills_Deployment_Recommendation.md` and recommendation notes.
5. `UAM_Model_Framework.md`.
6. The active skill package anchors under `uam-bridge-skills/skills/active/uam-bridge-skills/`: `MANIFEST.yaml`, `KERNEL.md`, `CHAIN_ROUTER.md`, and the current target skill.
...
1. Satisfy any active runtime requirement to read an installed skill, but treat that installed/global skill as a trigger shim only. Do not use its procedure, output contract, examples, or closeout shape as the operative source for UAM Bridge project work when a canonical project source file exists.
2. Immediately read the canonical project source for the invoked command before substantive analysis, routing, artifact creation, scoring interpretation, or closeout:
   - `uam-bridge-skills/skills/active/uam-bridge-skills/KERNEL.md`
   - `uam-bridge-skills/skills/active/uam-bridge-skills/MANIFEST.yaml` when manifest, authority, status, install, adapter, eval, or release limits matter
   - `uam-bridge-skills/skills/active/uam-bridge-skills/commands/<command-skill>/SKILL.md`
   - `uam-bridge-skills/skills/active/uam-bridge-skills/CHAIN_ROUTER.md` whenever continuation, next-step authority, stop gates, evidence upgrade risk, context handoff, or material routing behavior matters
3. Do not load additional profile-wide/general skills for supporting operations merely because their names overlap with project commands. For support roles, load the corresponding canonical project skill instead. Load a profile-wide skill only when the active runtime explicitly requires it, and then immediately return to the canonical project source.
4. Treat the canonical project source as the source-bound operating contract for this workspace unless a higher-priority instruction conflicts.
5. If the installed/global skill and the canonical project skill differ, name the mismatch when it materially affects routing, scope, closeout, or next-step selection.
6. Label the behavior as `source-bound dry-run` or `design-time/static` as appropriate. Do not claim the patched skills are installed, activated, runtime-verified, or globally controlling the agent.
7. If the canonical command skill or chain-router reference cannot be loaded, mark `source_basis_missing` and stop before material routing decisions that depend on it.

This `AGENTS.md` is the workspace operating overlay. The canonical package files own command behavior, router mechanics, manifest authority, adapter constraints, and eval/release gates unless this file or the user's current request sets a higher-priority workspace constraint.

For install or dogfood discussions, distinguish three states:

- `source-bound dry-run`: canonical files in this repository are read directly and control behavior for this workspace only.
- `isolated dogfood profile`: project-specific Codex skill files have been copied into a separately named non-global profile after explicit authorization, backup, hash recording, and smoke checks.
- `global/profile-wide skill`: installed outside the project-specific dogfood profile; never authoritative for UAM Bridge project behavior unless the user explicitly changes the governing package and install status.

Name-overlap rule: when a command name exists both in the global/profile-wide skill set and in `uam-bridge-skills/skills/active/uam-bridge-skills/commands/`, prefer the active project file for all UAM Bridge project reasoning, scoring, patches, and next-step recommendations. The profile-wide file is not evidence of the refined source behavior.
...
For each target skill, use this loop unless the user gives a narrower instruction or the work is a local, reversible correction whose dependencies are already known. For low-risk local corrections, use the fast path: read the target and directly affected governing context, patch only the named defect, verify by read-back or targeted static check, and record the limited evidence ceiling.
...
`uam-bridge-skills/docs/governance/source-authority-change-log.md` is the canonical narrative record for changes to source-authority resources. It explains why authority changed; artifact stamps, archives, integrity files, registries, manifests, and source control commits explain what bytes or paths changed.

Update the changelog in the same transaction whenever a change materially affects `AGENTS.md`, `UAM_Artifact_Index.yaml`, the active package `MANIFEST.yaml`, `KERNEL.md`, `CHAIN_ROUTER.md`, active command `SKILL.md` files, `SKILL_REGISTRY.yaml`, `TEST_REGISTRY.yaml`, governance/install/publication docs, file-placement maps, or active tools that control source authority, publication, install, export, validation, or QC behavior.

For indexed root authority artifacts, the changelog supplements the archive-replace-restamp-reindex transaction. Archive the prior active file, restamp the active file, mirror the new stamp and archive pointer in `UAM_Artifact_Index.yaml`, and record the reason, changed files, archive reference, hash or integrity reference, validation, evidence ceiling, prohibited claims, and follow-up in the changelog.

If the changelog cannot be updated during a source-authority change, stop or mark the change as incomplete. Do not let an unrecorded authority mutation become the new quiet baseline.
