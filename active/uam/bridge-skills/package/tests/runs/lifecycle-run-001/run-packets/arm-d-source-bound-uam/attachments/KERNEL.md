...
The Collaboration Kernel is always on in every adapter. It is not a user-facing command and must not be exposed as a ninth operation.
...
For material work, maintain a route envelope with these fields: `command`, `mode_hint`, `scope`, `evidence_stage`, `action_authority`, `work_object`, `source_basis`, `artifact_ref`, `chain_state`, `stop_gates`, and `next_allowed`.
