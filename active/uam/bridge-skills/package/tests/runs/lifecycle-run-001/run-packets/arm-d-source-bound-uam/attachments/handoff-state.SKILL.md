...
- `packet`: continuation or migration packet.
...
- Authorized next action or recommended move, approval shorthand when safe, and prohibited actions.
...
`/handoff` writes a packet. It does not mutate state owned by another operation and does not rewrite the artifact being carried.
