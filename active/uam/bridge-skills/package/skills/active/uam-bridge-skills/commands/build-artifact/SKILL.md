...
Completion-integrity rule: claim `created`, `wrote`, `changed`, `updated`, or `file write completed` only when a file write or equivalent provider action was actually observed. If the current context is simulated, text-only, raw-output-only, or lacks a tool/action trace, treat filesystem mutation as unobserved and report the artifact as `not written`, `contents returned only`, or `write unverified` as applicable. Do not carry an unverified artifact as created state into a later review, handoff, or continuation packet.
...
7. If file writing is expected but no write action is observed in the current context, do not claim completion. Return intended contents or a prepare-not-execute note and mark the artifact state `write unobserved`.
8. If a write action happened but no readback or equivalent verification happened, report `written but not verified`; do not claim content match, readiness, or handoff-resolved artifact state.
...
For file materialization, the change record must state one of these artifact states: `written and verified`, `written but not verified`, `write attempted and failed`, `write unobserved`, `contents returned only`, or `not written`. A later route may treat the artifact as loadable only for `written and verified`, or as explicitly unverified when the user knowingly continues from a weaker state.
...
Use `Changed:` only for an observed artifact mutation. For content-only or simulated outputs, use `Prepared:`, `Intended:`, `Contents:`, or `Not written:` instead.
...
If file writing cannot be observed from the current provider surface, do not infer that the file was created. Mark the state `write unobserved` or `contents returned only`, and name the verification needed before another command may rely on the artifact path.
