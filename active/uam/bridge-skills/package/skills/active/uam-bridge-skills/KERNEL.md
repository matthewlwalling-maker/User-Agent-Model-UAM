...
Load `CHAIN_ROUTER.md` before acting when a route involves bounded continuation, next-step authority, stop gates, review or research owed, artifact acting references, evidence-stage upgrade risk, context handoff, or material changes to routing, autonomy, or stop-gate policy. If the reference is unavailable, proceed only within this kernel policy and mark the reference gap in closeout.
