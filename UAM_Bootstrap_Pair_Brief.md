...
**Draft basis:** reconciled candidate framework and source-material Architect instructions recorded below; adopted parent authority remains mandatory at dispatch  
...
**Architect instructions source placement:** `AGENTS.md`; no standalone active instruction file is permitted.
...
These are provenance references, not dispatch authority. The frozen dispatch packet MUST replace them with the exact **adopted** parent stamp, adoption record, and loadable immutable content. A bare name, mutable path, numeric legacy label, or locally recomputed hash is insufficient.
...
|---|---|
| Framework authority | `UAM-FRAMEWORK` is adopted; exact stamp, adoption record, and loadable immutable content are attached. |
...
2. Each candidate declares a stable ID, intended versionless active filename, candidate content hash, parent framework pin, ownership, work-object authority, activation, composition, failure model, validation, and stopping.
...
|---|---|
| Adopted framework | Full adopted `UAM-FRAMEWORK` snapshot, stamp, content hash, and adoption record. |
...
Every source record must include its stable or legacy identity, exact content hash, loadable path or attached content, authority scope, and permitted use. A filename alone is not source provisioning.
...
- all acting-reference stamps and content hashes;
